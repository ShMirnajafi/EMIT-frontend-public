...
const MessageCard: React.FC<MessageCardProps> = ({type, message}) => {
    ...
    return (
        <div className={`rounded-xl shadow-lg w-full max-w-md border-transparent border-x-4 ${
            isSuccess ? 'bg-[#EAF8ED] border-x-[#38A169]' : 'bg-[#FEECEC] border-x-[#E53E3E]'
        }`}>
            <div className='p-6'>
                <div className="flex justify-end items-center">
                    <p className={`mx-8 text-right text-xl font-bold ${isSuccess ? 'text-[#228B22]' : 'text-[#E53E3E]'}`}>{message}</p>
